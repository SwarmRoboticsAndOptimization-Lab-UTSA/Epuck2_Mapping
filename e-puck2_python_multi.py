    ...
    while True:
        ...
        while(expected_recv_packets > 0):
            ...
            if header == bytearray([1]): # Get a QQVGA image
                ...
                #From proximity[client_index][0] - proximity[client_index][7]  
                print("prox0 = " + str(proximity[client_index][0])) #First sensor front side right of the camera
                print("prox1 = " + str(proximity[client_index][1])) #Second sensor to the right of the camera
                print("prox2 = " + str(proximity[client_index][2])) #Third sensor to the right of the camera
                print("prox3 = " + str(proximity[client_index][3])) #Fourth sensor to the right of the camera
                
                print("prox4 = " + str(proximity[client_index][4])) #Fourth sensor to the left of the camera
                print("prox5 = " + str(proximity[client_index][5])) #Third sensor to the left of the camera
                print("prox6 = " + str(proximity[client_index][6])) #Second sensor to the left of the camera
                print("prox7 = " + str(proximity[client_index][7])) #First sensor to the left of the camera
                
                # Here goes your controller.

                # print(proximity[client_index][0])
                # print(type(proximity[client_index][0]))
                # print(SENS_THRESHOLD)
                if ((proximity[client_index][0] >= SENS_THRESHOLD) or 
                    (proximity[client_index][1] >= SENS_THRESHOLD) or 
                    (proximity[client_index][2] >= SENS_THRESHOLD) or
                    (proximity[client_index][3] >= SENS_THRESHOLD)):
                ...
                elif((proximity[client_index][4] >= SENS_THRESHOLD) or 
                     (proximity[client_index][5] >= SENS_THRESHOLD) or 
                     (proximity[client_index][6] >= SENS_THRESHOLD) or
                     (proximity[client_index][7] >= SENS_THRESHOLD)):
                    right_motor_LSB, right_motor_MSB = get_motor_bytes(0)
                    left_motor_LSB, left_motor_MSB = get_motor_bytes(0)
                    command[client_index][3] = left_motor_LSB		# left motor LSB
                    command[client_index][4] = left_motor_MSB		# left motor MSB
