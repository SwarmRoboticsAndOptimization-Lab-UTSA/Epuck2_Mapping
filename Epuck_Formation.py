    ...
    while True:
        ...
        while(expected_recv_packets > 0):
            ...
            if header == bytearray([1]): # Get a QQVGA image
                ...
                # Desired speeds
                des_speed_right = 200
                des_speed_left = 200 #500 FORWARD 400 TURN LEFT WHILE MOVING FORWARD
                ...
                if loop_100 >= 100:
                    ...
                    delta_x = location[0] - prev_location[0]
                    delta_y = location[1] - prev_location[1]
                    ...
                    # Calculate the difference in each coordinate
                    delta_x1 = location[0] - desired_location[0]
                    delta_y1 = location[1] - desired_location[1]
                    # Calculate heading (angle in radians)
                    desired_heading = np.arctan2(delta_y1, delta_x1)
                    
                    # Convert heading to degrees if needed
                    desired_heading_deg = np.degrees(desired_heading)


                    print(f"Heading (radians): {heading}, Heading (degrees): {heading_deg}")
                    print(f"desired_heading (radians): {desired_heading}, desired_heading_deg (degrees): {desired_heading_deg}")
                ...
                # print(f"Error: {error_left}, Left Speed: {des_speed_left}, Right Speed: {des_speed_right}")
                ...
                #cv2.imshow('PID Controller', np.zeros((1, 512, 3), np.uint8))
